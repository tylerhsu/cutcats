...
export class ReportForm extends React.Component {
  ...
  renderDownloadLinks () {
    ...
    return (
      <div className="my-4">
        <div>
          <a href={`/api/rides/csv?${query}`} target="_blank" rel="noopener noreferrer">Download rides for {this.getDateString()}</a>
        </div>
        <div>
          <a href={`/api/payroll/csv?${query}`} target="_blank" rel="noopener noreferrer">Download payroll for {this.getDateString()}</a>
        </div>
        <div>
          <a href={`/api/invoices/csv?${query}`} target="_blank" rel="noopener noreferrer">Download invoices for {this.getDateString()}</a>
        </div>
      </div>
    );
  }
