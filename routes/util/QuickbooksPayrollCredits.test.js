...
describe('QuickbooksPayrollCredits', function() {
  ...
  describe('this.getFeeRow()', function() {
    ...
    it('returns an object with expected values', function() {
      const row = this.quickbooksPayrollCredits.getFeeRow(this.courierPaystubs[0], 1);
      row.should.eql({
        'RefNumber': 1,
    ...
    it('throws an error if the second argument is not supplied', function() {
      (() => {
        this.quickbooksPayrollCredits.getFeeRow(this.courierPaystubs[0]);
  ...
  describe('this.getTipRow()', function() {
    ...
    it('returns an object with expected values', function() {
      const row = this.quickbooksPayrollCredits.getTipRow(this.courierPaystubs[0], 1);
      row.should.eql({
        'RefNumber': 1,
        'CutCat Name': 'fixture courier',
        'Date': '01/31/2000',
        'Expense Account': 'Guaranteed Pay to Partners:Delivery Fee Payout:Tips',
        'Expense Amount': '3.00',
        'Expense Description': 'Invoiced Rides Tips Payout pay period 01/01/2000-01/31/2000',
        'Expense Class': 'CutCats',
        'AP Account': 'Accounts Payable'
      });
    });
    ...
    it('throws an error if the second argument is not supplied', function() {
      (() => {
        this.quickbooksPayrollCredits.getTipRow(this.courierPaystubs[0]);
      }).should.throw(/refNumber is required/);
    });
  });
});
